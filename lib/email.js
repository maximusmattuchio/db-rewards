...
// Vercel env var is currently named KLAYVIO_API_KEY (legacy typo). Try both so the
// fix is silent whether the env name is corrected or not.
const KLAVIYO_API_KEY = process.env.KLAVIYO_API_KEY || process.env.KLAYVIO_API_KEY;
...
// Send a plain-text alert to the admin — used when emails/webhooks die permanently
async function sendAdminAlert(subject, body) {
  if (!GMAIL_USER || !GMAIL_PASS) {
    console.error('[email] sendAdminAlert: Gmail not configured, cannot send alert');
    return;
  }
  try {
    await getGmailTransporter().sendMail({
      from: FROM_EMAIL,
      to: ADMIN_EMAIL,
      subject: `[Dirty Bastard Alert] ${subject}`,
      text: body,
    });
...
module.exports = { sendRewardEmail, trackCycleEvent, sendAdminAlert };
